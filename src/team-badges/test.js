  ...
  describe('given result with properties', () => {
    ...
    beforeEach(async () => {
      someBadge = {
        ...
        'image': 'https://www.funfunforum.com/uploads/funfunforum/original/2X/a/a0ec690519e29be286f35fb495b4555cd47426db.png',
        'enabled': true,
    ...
    it('picks description', () => expect(result[0].description).toBe(someBadge.description))
    it('picks image', () => expect(result[0].image).toBe(someBadge.image))
    it('picks slug', () => expect(result[0].slug).toBe(someBadge.slug))
    it('DOES NOT pick badge_grouping_id', () => expect(result[0].badge_grouping_id).toBeUndefined())
    it('DOES NOT pick other properties', () => expect(result[0]['other-prop']).toBeUndefined())
  })
})
